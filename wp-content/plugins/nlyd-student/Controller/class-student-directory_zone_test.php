...
class Student_Directory_Zone_Test
{
    ...
    public function __construct()
    {
        ...
        //添加短标签
        add_shortcode('directory_zone_test-home',array($this,'index'));
    }
    ...
    public function index(){
        ...
        $file_name = $dir.'/zone_test.html';
        if(file_exists($json_file)){
            $conf = json_decode(file_get_contents($json_file), true);
        }else{
            $conf['zoneTestTime'] = '';
            $conf['zoneTestTermTime'] = '';
        }
        global $wpdb;
        $view = $this->template_dir.'/directory-zone_pc.php';
        $type_id = $wpdb->get_var("SELECT id FROM {$wpdb->prefix}zone_type WHERE zone_type_alias='test'");
        ...
        if($conf['zoneTestTime'] == $last_time && $conf['zoneTestTermTime'] > $current_time && file_exists($file_name)){
...
//            leo_dump($rows);die;
            ...
            if($rows) {
                ...
                $conf['zoneTestTime'] = $last_time;
                $conf['zoneTestTermTime'] = get_time()+3600*24*30;
            }
