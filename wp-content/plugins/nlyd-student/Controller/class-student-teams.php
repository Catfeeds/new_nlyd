...
//        die;
    ...
    public function myCoach(){
        ...
        $rows = $wpdb->get_results("SELECT my.coach_id,p.post_title,my.category_id 
                FROM `{$wpdb->prefix}my_coach` AS my 
        ...
        foreach ($rows as $row){
            ...
            $category = [
                ...
                'category_name' => $row['post_title'],
            ];
            if(isset($datas[$row['coach_id']])){
                $datas[$row['coach_id']]['category'][] = $category;
            }else{
                $datas[$row['coach_id']] = [];
...
//        leo_dump($datas);die;
        ...
        load_view_template($view,['rows'=>$datas]);
    }
