...
class Student_Directory_D
{
    ...
    public function index(){
        ...
        if(($data = $redis->get('readDirectory'))){
        ...
        if(empty($rows)){
            ...
            $redis->setex('readDirectory', 3600*24*30,json_encode(['last_time'=>$last_time,'rows'=>$rows]));
        }
        load_view_template($this->template_dir.'directory-read_pc.php', ['rows'=>$rows]);
    }
