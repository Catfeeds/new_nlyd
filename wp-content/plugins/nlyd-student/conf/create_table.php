    ...
    **************示例*****************
    ...
    if($wpdb->get_var("show tables like $table_name") != $table_name) {  //判断表是否已存在
        ...
        $sql = "CREATE TABLE " . $table_name . " (
          ...
          `provide_id` int(20) DEFAULT NULL COMMENT '付款人id',
          `match_id` int(20) DEFAULT NULL COMMENT 'order表match_id',
    ...
    if($wpdb->get_var("show tables like $table_name") != $table_name) {  //判断表是否已存在
        ...
        $sql = "CREATE TABLE " . $table_name . " (
          ...
          `manager_income` decimal(10,2) DEFAULT NULL COMMENT '事业员收益',
          `indirect_manager_id` int(20) DEFAULT NULL COMMENT '二级事业员(事业部长)',
          `indirect_manager_income` decimal(10.2) DEFAULT NULL COMMENT '二级事业员 收益',
          `income_status` tinyint(1) unsigned DEFAULT 1 COMMENT '1待确认,2已确认',
          `created_time` datetime DEFAULT NULL,
          PRIMARY KEY (`id`)
          )ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8;";
    ...
    if($wpdb->get_var("show tables like $table_name") != $table_name) {  //判断表是否已存在
        ...
        $sql = "CREATE TABLE " . $table_name . " (
          ...
          `user_type` varchar(2) NOT NULL COMMENT '用户类型 1:学员',
    ...
    if($wpdb->get_var("show tables like $table_name") != $table_name) {  //判断表是否已存在
        ...
        $sql = "CREATE TABLE " . $table_name . " (
          `id` int(20) unsigned NOT NULL AUTO_INCREMENT,
          `user_id` int(20) unsigned NOT NULL COMMENT '机构账户id',
          `team_id` int(20) unsigned NOT NULL COMMENT '战队id',
          `team_world` varchar(255) DEFAULT NULL COMMENT '战队国籍',
          `team_slogan` text COMMENT '战队口号',
          `team_director` int(20) unsigned DEFAULT NULL COMMENT '战队负责人',
          ...
          `parent_id` int(20) unsigned DEFAULT 0 COMMENT '上级战队id(team_id)',
...
//    }
    ...
    $table_name = $wpdb->prefix . "user_extract_logs";  //用户提现记录表

    if($wpdb->get_var("show tables like $table_name") != $table_name) {  //用户提现记录表
        $sql = "CREATE TABLE `{$table_name}` (
          `id` int(20) unsigned NOT NULL AUTO_INCREMENT,
          `stream_log_id` int(20) unsigned NOT NULL COMMENT '流水记录id',
