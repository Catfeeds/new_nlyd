...
//        add_action('admin_enqueue_scripts', array($this, 'register_scripts'));
    ...
    public function register_teacher_menu_page(){
        ...
        if ( current_user_can( 'administrator' ) && !current_user_can( 'spread' ) ) {
            global $wp_roles;
            $role = 'spread_lxl_course';//权限名
            $wp_roles->add_cap('administrator', $role);

            $role = 'spread_add_lxl_course';//权限名
            $wp_roles->add_cap('administrator', $role);

        }
        add_submenu_page('spread','乐学乐课程','乐学乐课程','spread_lxl_course','spread-lxl_course',array($this,'lxlCourseList'));
        add_submenu_page('spread','新增乐学乐','新增乐学乐','spread_add_lxl_course','spread-add_lxl_course',array($this,'addLxlCourse'));
    }

    /**
     * 乐学乐课程
     */
    public function lxlCourseList(){
        global $wpdb;
        $page = isset($_GET['cpage']) ? intval($_GET['cpage']) : 1;
        $page < 1 && $page = 1;
        $pageSize = 20;
        $start = ($page-1)*$pageSize;
        $rows = $wpdb->get_results("SELECT SQL_CALC_FOUND_ROWS * FROM `{$wpdb->prefix}lxl_course` LIMIT {$start},{$pageSize}", ARRAY_A);
    ...
    public function addLxlCourse(){
        ...
        if(is_post()){
            ...
            if($msg == ''){
                ...
                $wpdb->startTrans();
                $insertRes = $wpdb->insert($wpdb->prefix.'lxl_course', $insertData);
                if($insertRes){
                    $id = $wpdb->insert_id;
                    ...
                    if($file){
                        $bool = $wpdb->update($wpdb->prefix.'lxl_course',['course_img'=>$upload_dir['baseurl'].$dir.$file],['id'=>$id]);
                        if($bool){
                            $success = '添加成功';
                            $wpdb->commit();
                        }else{
