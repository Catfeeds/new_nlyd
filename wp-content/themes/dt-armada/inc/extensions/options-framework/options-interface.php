...
function optionsframework_fields() {
	...
	foreach ( $options as $value ) {
		...
		case "fields_generator":
			...
			$del_link = '<div class="submitbox"><a href="#" class="of_fields_gen_del submitdelete">'. _x('Delete', 'backend fields', 'presscore'). '</a></div>';
			...
			$button = dt_create_tag( 'button', array(
				...
				'title' => isset($value['options']['button']['title'])?$value['options']['button']['title']:_x('Add', 'backend fields button', 'presscore'),
		...
		case "fields_generator_alpha":
			...
			if ( is_array( $val ) ) {
				...
				foreach ( $val as $field_id=>$field_data ) {
					...
					$title = empty($field_data['title']) ? _x('no title', 'theme-options', 'presscore') : $field_data['title'];
		...
		case 'sortable':
			...
			case 'pages_list':
				$html = wp_dropdown_pages( array(
					...
					'id' => esc_attr( $value['id'] ),
					'echo' => 0,
					'show_option_none' => __( '&mdash; Select &mdash;', 'presscore' ),
					'option_none_value' => '0',
					'selected' => $val
				) );
