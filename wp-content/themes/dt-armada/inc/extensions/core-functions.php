...
function dt_core_send_mail() {
	...
	$fields_titles = array(
		'name'		=> _x( 'Name: ', 'mail', 'presscore' ),
		'email'		=> _x( 'E-mail: ', 'mail', 'presscore' ),
		'telephone'	=> _x( 'Telephone: ', 'mail', 'presscore' ),
		'country'	=> _x( 'Country: ', 'mail', 'presscore' ),
		'city'		=> _x( 'City: ', 'mail', 'presscore' ),
		'company'	=> _x( 'Company: ', 'mail', 'presscore' ),
		'website'	=> _x( 'Website: ', 'mail', 'presscore' ),
		'message'	=> _x( 'Message: ', 'mail', 'presscore' ),
	);
	...
	if ( ! check_ajax_referer( 'dt_contact_form', 'nonce', false ) ) {
		$errors = _x( 'Nonce do not match', 'feedback msg', 'presscore' );
	} elseif ( 2 == $check ) {
		$errors = _x( 'Captcha filled incorrectly', 'feedback msg', 'presscore' );
	} elseif ( 3 == $check ) {
		$errors = _x( 'Fill the captcha', 'feedback msg', 'presscore' );
		...
		// message
		if ( $send ) {
			$errors = _x( 'Feedback has been sent to the administrator', 'feedback msg', 'presscore' );
		} else {
			$errors = _x( 'The message has not been sent', 'feedback msg', 'presscore' );
		}
		$nonce = wp_create_nonce( 'dt_contact_form' );

	} elseif( $honey_msg ) {
		$errors = _x( 'Sorry, we suspect that you are bot', 'feedback', 'presscore' );
	}
...
function dt_count_words( $text, $num_words = 55 ) {
	...
	if ( 'characters' == _x( 'words', 'word count: words or characters?', 'presscore' ) && preg_match( '/^utf\-?8$/i', get_option( 'blog_charset' ) ) ) {
