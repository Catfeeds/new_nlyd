	...
	class RWMB_Image_Advanced_MK2_Field extends RWMB_Image_Field
	{
		...
		static function wp_ajax_attach_media()
		{
			...
			if ( empty($attachments_ids) ) {
				RW_Meta_Box::ajax_response( __('Empty atachments', 'presscore'), 'error' );
				exit;
			}
		...
		static function wp_ajax_reorder_images()
		{
			...
			RW_Meta_Box::ajax_response( __( 'Order saved', 'presscore' ), 'success' );
			exit;
		}
		...
		static function wp_ajax_delete_file()
		{
			...
			if ( $ok )
				...
				RW_Meta_Box::ajax_response( __( 'Error: Cannot delete file', 'presscore' ), 'error' );
			exit;
		}
		...
		static function html( $html, $meta, $field )
		{
			$i18n_title = apply_filters( 'rwmb_image_advanced_select_string', _x( 'Select or Upload Images', 'image upload', 'presscore' ), $field );
		...
		{
			$i18n_delete = apply_filters( 'rwmb_image_delete_string', _x( 'Delete', 'image upload', 'presscore' ) );
			$i18n_edit   = apply_filters( 'rwmb_image_edit_string', _x( 'Edit', 'image upload', 'presscore' ) );
			$li = '
				<li id="item_%s">
					<img src="%s" />
...
if ( ! class_exists( 'RWMB_Fancy_Category_Field' ) ) {
	class RWMB_Fancy_Category_Field {
		...
		static function html( $html, $meta, $field ) {
			...
			if ( in_array( $field['mode'], array('both', 'taxonomy') ) ) {
				...
				if ( $field['taxonomy'] ) {
					...
					self::$tax_query = get_terms( array(
		...
		static function get_posts_tab() {
			...
			$imgs_text = array(
				_x('no pictures', 'backend', 'presscore'),
				_x('1 picture', 'backend', 'presscore'),
				_x('% pictures', 'backend', 'presscore')
			);
			...
			foreach ( self::$posts_query->posts as $_post ) {
				...
				if( !is_wp_error($terms) && $terms ) { 
					...
					$terms_list = '<p><strong>' . _x('Categories: ', 'backend', 'presscore') . '</strong>' . implode( ', ', $term_links ) . '</p>';
				}else {
					$terms_list = '<p></p>';
				}
		...
		static function get_main_tab() {
			...
			$html .= '<h4>' . _x('You have:', 'backend', 'presscore') . '</h4>';
		...
		static function get_tabs() {
			$select = array(
				'all'       => _x('All', 'backend metabox', 'presscore'),
				'only'      => _x('Only', 'backend metabox', 'presscore'),
				'except'    => _x('All, except', 'backend metabox', 'presscore'),
			);

			$type = array(
				'albums'		=> _x('Albums', 'backend metabox', 'presscore'),
				'category'		=> _x('Category', 'backend metabox', 'presscore'),
			);
			...
			$html .= '<div class="dt_tabs">';
				...
				$html .= '<div class="dt_arrange-by' . $hidden_class . '">';
					...
					if ( 'both' == self::$field['mode'] ) {
						$html .= '<strong>' . _x('Arrange by:', 'backend metabox', 'presscore') . '</strong>';
					}
	...
	class RWMB_Simple_Proportions_Field
	{
		...
		static function html( $html, $meta, $field )
		{
			...
			return sprintf(
				...
				_x( 'width', 'metabox', 'presscore' ),
				$width_name,
				$field['id'],
				$meta['width'],
				$field['size'],
				!$field['datalist'] ?  '' : "list='{$field['datalist']['id']}'",
				_x( 'height', 'metabox', 'presscore' ),
				$height_name,
				$field['id'],
				$meta['height'],
				$field['size'],
