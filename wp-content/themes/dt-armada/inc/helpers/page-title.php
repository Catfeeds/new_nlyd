...
if ( ! function_exists( 'presscore_get_page_title' ) ) :
	...
	function presscore_get_page_title() {
		...
		if ( is_page() || is_single() ) {
			...
			$title = sprintf( _x( 'Search Results for: %s', 'archive template title', 'presscore' ), '<span>' . get_search_query() . '</span>' );

		} else if ( is_archive() ) {

			if ( is_category() ) {
				$title = sprintf( _x( 'Category Archives: %s', 'archive template title', 'presscore' ), '<span>' . single_cat_title( '', false ) . '</span>' );

			} elseif ( is_tag() ) {
				$title = sprintf( _x( 'Tag Archives: %s', 'archive template title', 'presscore' ), '<span>' . single_tag_title( '', false ) . '</span>' );

			} elseif ( is_author() ) {
				the_post();
				$title = sprintf( _x( 'Author Archives: %s', 'archive template title', 'presscore' ), '<span class="vcard"><a class="url fn n" href="' . esc_url( get_author_posts_url( get_the_author_meta( "ID" ) ) ) . '" title="' . esc_attr( get_the_author() ) . '" rel="me">' . get_the_author() . '</a></span>' );
				rewind_posts();

			} elseif ( is_day() ) {
				$title = sprintf( _x( 'Daily Archives: %s', 'archive template title', 'presscore' ), '<span>' . get_the_date() . '</span>' );

			} elseif ( is_month() ) {
				$title = sprintf( _x( 'Monthly Archives: %s', 'archive template title', 'presscore' ), '<span>' . get_the_date( 'F Y' ) . '</span>' );

			} elseif ( is_year() ) {
				$title = sprintf( _x( 'Yearly Archives: %s', 'archive template title', 'presscore' ), '<span>' . get_the_date( 'Y' ) . '</span>' );

			} elseif ( is_tax('dt_portfolio_category') ) {
				$title = sprintf( _x( 'Portfolio Archives: %s', 'archive template title', 'presscore' ), '<span>' . single_term_title( '', false ) . '</span>' );

			} elseif ( is_tax('dt_gallery_category') ) {
				$title = sprintf( _x( 'Albums Archives: %s', 'archive template title', 'presscore' ), '<span>' . single_term_title( '', false ) . '</span>' );

			} else {
				$title = _x( 'Archives:', 'archive template title', 'presscore' );

			}

		} elseif ( is_404() ) {
			$title = _x( 'Page not found', 'index title', 'presscore' );

		} else {
			$title = _x( 'Blog', 'index title', 'presscore' );
