...
if ( ! function_exists( 'presscore_get_breadcrumbs' ) ) :
	...
	function presscore_get_breadcrumbs( $args = array() ) {
		...
		$default_args = array(
			'text' => array(
				'home' => _x('Home', 'breadcrumbs', 'presscore'),
				'category' => _x('Category "%s"', 'breadcrumbs', 'presscore'),
				'search' => _x('Results for "%s"', 'breadcrumbs', 'presscore'),
				'tag' => _x('Entries tagged with "%s"', 'breadcrumbs', 'presscore'),
				'author' => _x('Article author %s', 'breadcrumbs', 'presscore'),
				'404' => _x('Error 404', 'breadcrumbs', 'presscore')
			),
			'showCurrent' => 1,
			'showOnHome' => 1,
			'delimiter' => '',
		...
		$breadcrumbs_html .= '<div class="assistive-text">' . _x('You are here:', 'breeadcrumbs', 'presscore') . '</div>';
		...
		if (is_home() || is_front_page()) {
			...
			if ( get_query_var('paged') ) {
				...
				$breadcrumbs_html .= _x('Page', 'bredcrumbs', 'presscore') . ' ' . get_query_var('paged');
