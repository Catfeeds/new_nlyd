...
if ( ! function_exists( 'presscore_get_post_comments' ) ) :
	...
	function presscore_get_post_comments( $html = '' ) {
		if ( ! post_password_required() && ( comments_open() || '0' != get_comments_number() ) ) :
			ob_start();
			comments_popup_link( __( 'Leave a comment', 'presscore' ), __( '1 Comment', 'presscore' ), __( '% Comments', 'presscore' ), 'comment-link' );
			$html .= ob_get_clean();
		endif;
	...
	function presscore_post_details_link( $post_id = null, $class = array('details', 'more-link'), $link_text = null ) {
		...
		if ( $url ) {
			$output = sprintf(
				...
				is_string( $link_text ) ? $link_text : __( 'Read details', 'presscore' )
			);
		}
...
if ( ! function_exists( 'presscore_display_share_buttons' ) ) :
	...
	function presscore_display_share_buttons( $place = '', $options = array() ) {
		...
		$link_class = '';
		if ( ! $button_title ) {
			$button_title = __( 'Share this', 'presscore' );
			$link_class = ' no-text';
		}
...
if ( ! function_exists( 'presscore_display_post_author' ) ) :
	...
	function presscore_display_post_author() {
		...
		<div class="dt-fancy-separator title-left fancy-author-title">
			<div class="dt-fancy-title"><?php _e('About the author', 'presscore'); ?><span class="separator-holder separator-right"></span></div>
		</div>
...
if ( ! function_exists( 'presscore_get_post_media_count' ) ) :
	...
	function presscore_get_post_media_count( $html = '' ) {
		...
		if ( $images_count || $videos_count ) {
			...
			if ( $images_count ) {
				$counters[] = sprintf( _n( '1 image', '%s images', $images_count, 'presscore' ), $images_count );
			}

			if ( $videos_count ) {
				$counters[] = sprintf( _n( '1 video', '%s video', $videos_count, 'presscore' ), $videos_count );
			}
	...
	function presscore_get_post_author( $html = '' ) {
		$html .= sprintf(
			'<a class="author vcard" href="%s" title="%s" rel="author">%s<span class="fn">%s</span></a>',
				...
				esc_attr( sprintf( _x( 'View all posts by %s', 'frontend post meta', 'presscore' ), get_the_author() ) ), // title
				_x( 'By ', 'frontend post meta', 'presscore' ),
	...
	function dt_get_next_page_button( $max, $class = '' ) {
		...
		if ( $next_posts_link ) {
			...
			if ( presscore_is_lazy_loading() ) {
				$caption = __( 'Loading...', 'presscore' );
			} else {
				$caption = __( 'Load more', 'presscore' );
			}
...
if ( ! function_exists( 'presscore_get_terms_list_by_slug' ) ) :
	 ...
	 * Default titles: array( 'empty_slugs' => __( 'All', 'presscore' ), 'no_result' => __('There is no categories', 'presscore') )
	...
	function presscore_get_terms_list_by_slug( $args = array() ) {
		...
		$default_titles = array(
			'empty_slugs' => __( 'All', 'presscore' ),
			'no_result' => __('There is no categories', 'presscore')
		);
		...
		foreach ( $attachments_data as $data ) {
			...
			if ( 'none' != $link && $show_content ) {
				...
				if ( in_array('details', $options['fields']) ) {
					$html .= '<a class="project-details" href="' . esc_url($data['permalink']) . '">' . _x('Details', 'fullscreen slider two', 'presscore') . '</a>';
				}
	...
	function presscore_get_fullwidth_slider_two_with_hovers( $attachments_data, $options = array() ) {
		...
		foreach ( $attachments_data as $data ) {
			...
			if ( 'none' != $link && $show_content ) {
				...
				if ( in_array('details', $options['fields']) ) {
					$buttonts_count++;
					$post_buttons .= '<a class="project-details" href="' . esc_url($data['permalink']) . '">' . _x('Details', 'fullscreen slider two', 'presscore') . '</a>';
				}
	...
	function presscore_get_images_list( $attachments_data, $args = array() ) {
		...
		if ( $args['open_in_lightbox'] ) {
			...
			$base_img_args = array(
				...
				'img_class' => 'images-list',
				'echo' => false,
				'wrap' => '<a %HREF% %CLASS% title="%RAW_ALT%" data-dt-img-description="%RAW_TITLE%" %CUSTOM%><img %SRC% %IMG_CLASS% %ALT% style="width: 100%;" /></a>'
			);
...
if ( ! function_exists( 'presscore_get_images_gallery_hoovered' ) ) :
	...
	function presscore_get_images_gallery_hoovered( $attachments_data, $options = array() ) {
		...
		if ( $images_count ) {
			$count_text[] = sprintf( _n( '1 image', '%s images', $images_count, 'presscore' ), $images_count );
		}

		if ( $videos_count ) {
			$count_text[] = sprintf( __( '%s video', 'presscore' ), $videos_count );
		}
