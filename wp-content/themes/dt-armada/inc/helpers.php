...
require_once PRESSCORE_HELPERS_DIR . '/albums-helpers.php';
			...
			if ( $args['all_btn'] ) {
				...
				$all = str_replace(
					...
					array(
						...
						$all_class,
						_x( 'All posts', 'category list', 'presscore' ),
						_x( 'View all', 'category list', 'presscore' ),
						'',
						'',
						$data['all_count'],
						'*',
			...
			if( $data['other_count'] && $args['other_btn'] ) {
				...
				$output .= str_replace(
					...
					array(
						...
						$other_class,
						_x( 'Other posts', 'category list', 'presscore' ),
						_x( 'Other', 'category list', 'presscore' ),
						'',
						0,
						$data['other_count'],
						esc_attr('.category-0'),
...
if ( ! function_exists( 'pressocore_render_login_form' ) ) :
	...
	function pressocore_render_login_form() {
		...
		$caption = of_get_option( 'header-login_caption', _x( "Login", "theme-options", 'presscore' ) );
...
if ( ! function_exists( 'presscore_get_posts_small_list' ) ) :
	...
	function presscore_get_posts_small_list( $attachments_data, $options = array() ) {
		...
		foreach ( $attachments_data as $data ) {
			...
			if ( empty($data['title']) ) {
				$data['title'] = _x('No title', 'blog small list', 'presscore');
			}
