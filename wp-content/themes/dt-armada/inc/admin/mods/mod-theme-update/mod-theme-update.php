		...
		if ( $user || $api_key ) {
			...
			if ( $upgrader ) {
				...
				if ( !empty($responce->errors) ) {
					...
					add_settings_error( 'theme-update', 'update_errors', _x('Error:<br />', 'backend', 'presscore') . implode( '<br \>', $responce->errors ), 'error' );
				} else if ( $update_needed ) {

					// changelog link
					$message = sprintf( _x('New version (<a href="%s" target="_blank">see changelog</a>) of theme is available!', 'backend', 'presscore'), 'http://the7.dream-demo.com/changelog.txt' );

					// update link
					$message .= '&nbsp;<a href="' . esc_url( add_query_arg( 'theme-updater', 'update' ) ) . '">' . _x('Please, click here to update.', 'backend', 'presscore') . '</a>';

					add_settings_error( 'theme-update', 'update_nedded', $message, 'updated' );
				} else {

					add_settings_error( 'theme-update', 'theme-uptodate', _x("You're have most recent version of theme!", 'backend', 'presscore'), 'updated salat' );
				}
				...
				if ( $update_result ) {
					...
					if ( !empty($update_result->success) ) {
						...
						add_settings_error( 'theme-update', 'update_result', _x('Theme was successfully updated to newest version!', 'backend', 'presscore'), 'updated salat' );
					} else if ( !empty($update_result->installation_feedback) ) {

						add_settings_error( 'theme-update', 'update_result', _x('Error:<br />', 'backend', 'presscore') . implode('<br />', $update_result->installation_feedback), 'error' );
					}
				}
...
function presscore_theme_update() {
	...
	// regenrate stylesheets after succesful update
		...
		add_settings_error( 'options-framework', 'theme_updated', _x( 'Stylesheets regenerated.', 'backend', 'presscore' ), 'updated fade' );
