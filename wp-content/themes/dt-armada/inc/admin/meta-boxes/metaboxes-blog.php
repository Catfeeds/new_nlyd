...
$DT_META_BOXES[] = array(
	...
	'title' 	=> _x('Display Blog Categories', 'backend metabox', 'presscore'),
	'pages' 	=> array( 'page' ),
	'context' 	=> 'normal',
	'priority' 	=> 'high',
	'fields' 	=> array(
		...
		array(
			...
			'desc'				=> sprintf(
				...
				_x('ALL your Blog posts are being displayed on this page!', 'backend', 'presscore'),
				_x('By default all your Blog posts will be displayed on this page. ', 'backend', 'presscore'),
				_x('But you can specify which Blog categories will (or will not) be shown.', 'backend', 'presscore'),
				_x('In tabs above you can select from the following options:', 'backend', 'presscore'),

				_x( 'All', 'backend', 'presscore' ),

				_x(' &mdash; all Blog posts (from all categories) will be shown on this page.', 'backend', 'presscore'),

				_x( 'Only', 'backend', 'presscore' ),

				_x(' &mdash; choose Blog category(s) to be shown on this page.', 'backend', 'presscore'),

				_x( 'All, except', 'backend', 'presscore' ),

				_x(' &mdash; choose which category(s) will be excluded from displaying on this page.', 'backend', 'presscore')
			)
		)
	),
...
$DT_META_BOXES[] = array(
	...
	'title' 	=> _x('Blog Options', 'backend metabox', 'presscore'),
	'pages' 	=> array( 'page' ),
	'context' 	=> 'normal',
	'priority' 	=> 'high',
	'fields' 	=> array(
		...
		array(
			'name'				=> _x( 'Make all posts the same width:', 'backend metabox', 'presscore' ),
		...
		Presscore_Meta_Box_Field_Template::get_as_array( 'background under masonry post', array(
			'name'				=> _x( 'Background under posts:', 'backend metabox', 'presscore' ),
		...
		// Enable fancy date
		array(
			'name'			=> _x( 'Enable fancy date:', 'backend metabox', 'presscore' ),
		...
		array(
			'name'		=> _x( 'Number of posts to display on one page:', 'backend metabox', 'presscore' ),
			'id'		=> "{$prefix}ppp",
			'type'		=> 'text',
			'std'		=> '',
			'divider'	=> 'top'
		...
		// Show excerpts
		Presscore_Meta_Box_Field_Template::get_as_array( 'radio yes no', array( 'id' => "{$prefix}show_exerpts", 'name' => _x( 'Show excerpts:', 'backend metabox', 'presscore' ), 'divider' => 'top' ) ),

		// Show read more buttons
		Presscore_Meta_Box_Field_Template::get_as_array( 'radio yes no', array( 'id' => "{$prefix}show_details", 'name' => _x( 'Show read more buttons:', 'backend metabox', 'presscore' ), 'divider' => 'top' ) ),
		...
		// Show categories
		Presscore_Meta_Box_Field_Template::get_as_array( 'radio yes no', array( 'id' => "{$prefix}show_categories_in_post_meta", 'name' => _x( 'Show post categories:', 'backend metabox', 'presscore' ), 'divider' => 'top' ) ),

		// Show date
		Presscore_Meta_Box_Field_Template::get_as_array( 'radio yes no', array( 'id' => "{$prefix}show_date_in_post_meta", 'name' => _x( 'Show post date:', 'backend metabox', 'presscore' ) ) ),

		// Show author
		Presscore_Meta_Box_Field_Template::get_as_array( 'radio yes no', array( 'id' => "{$prefix}show_author_in_post_meta", 'name' => _x( 'Show post author:', 'backend metabox', 'presscore' ) ) ),

		// Show comments
		Presscore_Meta_Box_Field_Template::get_as_array( 'radio yes no', array( 'id' => "{$prefix}show_comments_in_post_meta", 'name' => _x( 'Show post comments:', 'backend metabox', 'presscore' ) ) ),
...
$DT_META_BOXES[] = array(
	...
	'title' 	=> _x('Post Options', 'backend metabox', 'presscore'),
	'pages' 	=> array( 'post' ),
	'context' 	=> 'normal',
	'priority' 	=> 'high',
	'fields' 	=> array(
		...
		array(
			'name'    		=> __('Hide featured image on post page:', 'presscore'),
		...
		array(
			'name'    	=> _x('Related posts category:', 'backend metabox', 'presscore'),
		...
		array(
			'name'    	=> _x('Post preview width:', 'backend metabox', 'presscore'),
			'id'      	=> "{$prefix}preview",
			'type'    	=> 'radio',
			'std'		=> 'normal',
			'options'	=> array(
			...
			'before'	=> '<p><small>' . sprintf(
				_x('Related posts can be enabled / disabled from %sTheme Options / Blog, Portfolio, Gallery%s', 'backend metabox', 'presscore'),
				'<a href="' . esc_url( add_query_arg( 'page', 'of-blog-and-portfolio-menu', get_admin_url() . 'admin.php' ) ) . '" target="_blank">',
				'</a>'
			) . '</small></p><div class="dt_hr"></div><p><strong>' . _x('Post Preview Options', 'backend metabox', 'presscore') . '</strong></p>',
		),

		// Preview gallery
		array(
			'name'    		=> _x('For gallery post format:', 'backend metabox', 'presscore'),
			'id'      		=> "{$prefix}preview_style_gallery",
			'type'    		=> 'radio',
			'std'			=> 'standard_gallery',
			'options'		=> array(
				'standard_gallery'	=> _x('standard image gallery', 'backend metabox', 'presscore'),
				'hovered_gallery' 	=> _x('featured image with gallery hover', 'backend metabox', 'presscore'),
				'slideshow'			=> _x('slideshow', 'backend metabox', 'presscore'),
			),
			'before'		=> '<div class="dt_hr"></div><p><strong>' . _x('Post Preview Style', 'backend metabox', 'presscore') . '</strong></p>',
			'hide_fields'	=> array(
		...
		// Preview video
		array(
			'name'    	=> _x('For video post format:', 'backend metabox', 'presscore'),
			'id'      	=> "{$prefix}preview_style_video",
			'type'    	=> 'radio',
			'std'		=> 'image_play',
			'options'	=> array(
				'image' 			=> _x('image', 'backend metabox', 'presscore'),
				'image_play'		=> _x('image with "Play" icon', 'backend metabox', 'presscore'),
			),
		),
