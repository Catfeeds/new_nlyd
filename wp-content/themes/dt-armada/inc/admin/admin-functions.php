...
function presscore_admin_notice() {
	...
	?>
	<div class="updated">
		<p><strong><?php echo _x( 'Failed to create customization .CSS file. To improve your site performance, please check whether "…/wp-content/uploads/" folder is created, and its CHMOD is set to 777.', 'backend css file creation error', 'presscore' ); ?></strong></p>
	</div>
	<?php
}
...
function presscore_attachment_fields_to_edit( $fields, $post ) {
	...
	if ( strpos( get_post_mime_type( $post->ID ), 'image' ) !== false ) {
		...
		$fields['dt-video-url'] = array(
				'label' 		=> _x('Video url', 'attachment field', 'presscore'),
				'input' 		=> 'text',
		...
		$fields['dt-img-link'] = array(
				'label' 		=> _x('Image link', 'attachment field', 'presscore'),
				'input' 		=> 'text',
	//			'html'       	=> "<input type='text' class='text widefat' name='attachments[$post->ID][dt-video-url]' value='" . esc_attr($img_link) . "' /><br />",
		...
		$fields['dt-img-hide-title'] = array(
				'label' 		=> _x('Hide title', 'attachment field', 'presscore'),
				'input' 		=> 'html',
...
function presscore_add_thumbnails_column_in_admin( $defaults ){
	...
	$head['presscore-thumbs'] = _x( 'Thumbnail', 'backend', 'presscore' );
...
function presscore_add_sidebar_and_footer_columns_in_admin( $defaults ){
	$defaults['presscore-sidebar'] = _x( 'Sidebar', 'backend', 'presscore' );
	$defaults['presscore-footer'] = _x( 'Footer', 'backend', 'presscore' );
	return $defaults;
}
...
function presscore_add_slug_column_for_slideshow( $defaults ){
	$defaults['presscore-slideshow-slug'] = _x( 'Slug', 'backend', 'presscore' );
	return $defaults;
}
...
function presscore_add_title_column_for_media( $columns ) {
	$columns['presscore-media-title'] = _x( 'Image title', 'backend', 'presscore' );
	return $columns;
}
...
function presscore_display_title_status_for_media( $column_name, $id ) {
	if ( 'presscore-media-title' == $column_name ) {
		...
		if ( $hide_title ) {
			echo '<span class="dt-media-hidden-title">' . _x('Hidden', 'media title hidden', 'presscore') . '</span>';
		} else {
			echo '<span class="dt-media-visible-title">' . _x('Visible', 'media title visible', 'presscore') . '</span>';
		}
	}
}
	...
	<div class="inline-edit-col-right" style="display: inline-block; float: left;">
		<fieldset>
			<div class="inline-edit-col">
				...
				<div class="inline-edit-group">
					<label class="alignleft">
						<span class="title"><?php _ex( 'Sidebar option', 'backend bulk edit', 'presscore' ); ?></span>
						<?php
						$sidebar_options = array(
							'left' 		=> _x('Left', 'backend bulk edit', 'presscore'),
							'right' 	=> _x('Right', 'backend bulk edit', 'presscore'),
							'disabled'	=> _x('Disabled', 'backend bulk edit', 'presscore'),
						);
						?>
						<select name="_dt_bulk_edit_sidebar_options">
							<option value="-1"><?php _ex( '&mdash; No Change &mdash;', 'backend bulk edit', 'presscore' ); ?></option>
					...
					<label class="alignright">
						<span class="title"><?php _ex( 'Widgetized footer', 'backend bulk edit', 'presscore' ); ?></span>
						<?php
						$show_wf = array(
							0	=> _x('Hide', 'backend bulk edit footer', 'presscore'),
							1	=> _x('Show', 'backend bulk edit footer', 'presscore'),
						);
						?>
			...
			<?php if ( function_exists('presscore_get_widgetareas_options') && $wa_list = presscore_get_widgetareas_options() ): ?>
				...
				<div class="inline-edit-group">
					<label class="alignleft">
						<span class="title"><?php _ex( 'Sidebar', 'backend bulk edit', 'presscore' ); ?></span>
					...
					<label class="alignright">
						<span class="title"><?php _ex( 'Footer', 'backend bulk edit', 'presscore' ); ?></span>
...
function presscore_add_media_bulk_actions() {
	...
	if ( $post_type == 'attachment' ) {
		$show_title_text = _x('Show titles', 'media bulk action', 'presscore');
		$hide_title_text = _x('Hide titles', 'media bulk action', 'presscore');
	?>
...
function presscore_media_bulk_actions_handler() {
	...
	if ( $post_type != '' ) {
		...
		$error_msg = _x('You are not allowed to perform this action.', 'backend media error', 'presscore');
...
if ( ! function_exists( 'presscore_setup_admin_scripts' ) ) :
	...
	function presscore_setup_admin_scripts( $hook ) {
		...
		wp_localize_script( 'dt-admin-widgets', 'dtWidgtes', array(
			'title'			=> _x( 'Title', 'widget', 'presscore' ),
			'content'		=> _x( 'Content', 'widget', 'presscore' ),
			'percent'		=> _x( 'Percent', 'widget', 'presscore' ),
			'showPercent'	=> _x( 'Show', 'widget', 'presscore' ),
		) );
