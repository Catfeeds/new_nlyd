...
class Presscore_Inc_Widgets_ProgressBars extends WP_Widget {
	...
	function __construct() { 
		/* Widget settings. */
		$widget_ops = array( 'description' => _x( 'Progress bars', 'widget', 'presscore' ) );
		...
		parent::__construct(
			...
			DT_WIDGET_PREFIX . _x( 'Progress bars', 'widget', 'presscore' ),
			$widget_ops,
			$control_ops
		);
	}
	...
	function widget( $args, $instance ) {
		...
		$percent_text = _x('%d&#37;', 'progress bat widget', 'presscore');
		...
		<div class="dt-widget-sortable-container">
			...
			?>
				...
				<li class="ui-state-default" data-index="<?php echo $index; ?>">
					...
					<input type="text" name="<?php echo $this->get_field_name( 'fields' ) . "[$index]"; ?>[title]" placeholder="<?php echo esc_attr( _x( 'Title', 'widget', 'presscore' ) ); ?>" value="<?php echo esc_attr( $field['title'] ); ?>" /><br />
					<input type="text" max="100" min="0" size="8" name="<?php echo $this->get_field_name( 'fields' ) . "[$index]"; ?>[percent]" placeholder="<?php echo esc_attr( _x( 'Percent', 'widget', 'presscore' ) ); ?>" value="<?php echo esc_attr( $field['percent'] ); ?>" />
					<label><input type="checkbox" name="<?php echo $this->get_field_name( 'fields' ) . "[$index]"; ?>[show_percent]" value="1" <?php checked($field['show_percent']); ?> />&nbsp;<?php echo esc_attr( _x( 'Show', 'widget', 'presscore' ) ); ?></label><br />
			...
			<?php endforeach;
			?></ul>
			<a href="javascript: void(0);" class="dt-widget-sortable-add button" data-fields-name="<?php echo $this->get_field_name( 'fields' ); ?>" data-field-type="progress-bar"><?php _ex( 'Add', 'widget', 'presscore' ); ?></a>
		</div>
