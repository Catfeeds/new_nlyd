...
class Presscore_Inc_Widgets_ContactInfo extends WP_Widget {
	...
	function __construct() {  
		/* Widget settings. */
		$widget_ops = array( 'description' => _x( 'Contact info', 'widget', 'presscore' ) );

		/* Create the widget. */
		parent::__construct(
			'presscore-contact-info-widget',
			DT_WIDGET_PREFIX . _x( 'Contact info', 'widget', 'presscore' ),
			$widget_ops
		);
	...
	function widget( $args, $instance ) {
		...
		if ( !empty($links) && implode('', (array)$links) ) {
			...
			echo '<div class="soc-ico"><p class="assistive-text">' . _x('Find us on:', 'widget', 'presscore') . '</p>';
		...
		<div class="dt-widget-sortable-container">
			<ul class="dt-widget-sortable ui-sortable">
				...
				<li class="ui-state-default" data-index="<?php echo $index; ?>">
					...
					<textarea class="widefat" name="<?php echo $this->get_field_name( 'fields' ) . "[$index]"; ?>[content]" placeholder="<?php echo esc_attr( _x( 'Content', 'widget', 'presscore' ) ); ?>"><?php echo esc_textarea( $field['content'] ); ?></textarea>
				</li>

			<?php endforeach; ?>

			</ul>
			<a href="javascript: void(0);" class="dt-widget-sortable-add" data-fields-name="<?php echo $this->get_field_name( 'fields' ); ?>" data-field-type="contact-info"><?php _ex( 'Add', 'widget', 'presscore' ); ?></a>
		</div>

		<p>
			<h4><?php _ex('Social links:', 'widget', 'presscore'); ?></h4>
