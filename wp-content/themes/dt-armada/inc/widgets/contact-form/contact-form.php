...
class Presscore_Inc_Widgets_ContactForm extends WP_Widget {
	...
	function __construct() {  
		/* Widget settings. */
		$widget_ops = array( 'description' => _x( 'Contact form', 'widget', 'presscore' ) );

		/* Create the widget. */
		parent::__construct(
			'presscore-contact-form-widget',
			DT_WIDGET_PREFIX . _x( 'Contact form', 'widget', 'presscore' ),
			$widget_ops
		);

		// fill fields list 
		self::$fields_list = array(
			'name'		=> array( 'title' => _x('Name', 'widget', 'presscore'), 'defaults' => array( 'on' => true, 'required' => true ) ),
			'email'		=> array( 'title' => _x('E-mail', 'widget', 'presscore'), 'defaults' => array( 'on' => true, 'required' => true ) ),
			'telephone'	=> array( 'title' => _x('Telephone', 'widget', 'presscore'), 'defaults' => array( 'on' => false, 'required' => false ) ),
			'country'	=> array( 'title' => _x('Country', 'widget', 'presscore'), 'defaults' => array( 'on' => false, 'required' => false ) ),
			'city'		=> array( 'title' => _x('City', 'widget', 'presscore'), 'defaults' => array( 'on' => false, 'required' => false ) ),
			'company'	=> array( 'title' => _x('Company', 'widget', 'presscore'), 'defaults' => array( 'on' => false, 'required' => false ) ),
			'website'	=> array( 'title' => _x('Website', 'widget', 'presscore'), 'defaults' => array( 'on' => false, 'required' => false ) ),
			'message'	=> array( 'title' => _x('Message', 'widget', 'presscore'), 'defaults' => array( 'on' => true, 'required' => false ) ),
		);
	...
	function widget( $args, $instance ) {
		...
		if ( $fields_not_empty ) {
			...
			$button_title = !empty($instance['button_title']) ? $instance['button_title'] : _x('Submit', 'widget', 'presscore');
			...
			echo presscore_get_button_html( array(
				'href' => '#',
				'title' => _x('clear', 'widget', 'presscore'),
				'class' => 'clear-form',
				'atts' => ' rel="nofollow"'
			) );

			echo '<input class="assistive-text" type="submit" value="' . esc_attr( _x('submit', 'widget', 'presscore') ) . '">';
	...
	function form( $instance ) {
		...
		?>
		<div>
			<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _ex('Title:', 'widget',  'presscore'); ?></label>
			<input type="text" id="<?php echo $this->get_field_id( 'title' ); ?>" class="widefat" name="<?php echo $this->get_field_name( 'title' ); ?>" value="<?php echo esc_attr($instance['title']); ?>" />
		</div>
		
		<div>
			<label for="<?php echo $this->get_field_id( 'text' ); ?>"><?php _ex('Text:', 'widget',  'presscore'); ?></label>
		...
		<h4><?php _ex('Fields:', 'widget', 'presscore'); ?></h4>
		...
		<p>
			...
			<label><input type="radio" name="<?php echo $this->get_field_name( 'fields' ) . "[$index]"; ?>[on]" value="1" <?php checked( $field_on ); ?> /><?php _ex('on', 'widget', 'presscore'); ?></label>
			<label><input type="radio" name="<?php echo $this->get_field_name( 'fields' ) . "[$index]"; ?>[on]" value="0" <?php checked( !$field_on ); ?> /><?php _ex('off', 'widget', 'presscore'); ?></label>&nbsp;
			<label><input type="checkbox" name="<?php echo $this->get_field_name( 'fields' ) . "[$index]"; ?>[required]" value="1" <?php checked($field_required); ?>/>&nbsp;<?php _ex('required', 'widget', 'presscore'); ?></label>
		</p>

		<?php endforeach; ?>

		<p>
			<label><?php _ex('Message field height (in lines)', 'widget', 'presscore'); ?>&nbsp;<input type="text" name="<?php echo $this->get_field_name( 'msg_height' ); ?>" value="<?php echo esc_attr($instance['msg_height']); ?>" size="3"/></label>
		</p>

		<p>
			<label><?php _ex('Send mail to:', 'widget', 'presscore'); ?><input type="text" class="widefat" name="<?php echo $this->get_field_name( 'send_to' ); ?>" value="<?php echo esc_attr($instance['send_to']); ?>"/></label>
		</p>       	
