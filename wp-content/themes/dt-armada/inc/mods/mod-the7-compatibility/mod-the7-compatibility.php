	...
	class Presscore_Mod_The7_Adapter {
		...
		public function add_admin_notices() {
			...
			if ( $this->the7_options_found() ) {
				...
				$msg = '<p>' 
						. sprintf( _x( 'The7 detected!<br/>Would you like to import The7 settings to %s ?', 'options import', 'presscore' ), wp_get_theme()->get('Name') ) 
					. '</p>' 
					. '<p>' 
						. '<label>' 
							. _x( 'Choose default skin:', 'options import', 'presscore' ) 
							. '&nbsp;' 
							. $presets_select 
						. '</label>' 
					. '</p>' 
					. '<div class="dt-buttons-holder">' 
						. '<div class="dt-button-secondary">' 
							. '<a href="' . esc_url( $dismiss_link ) . '" class="button button-secondary">' . _x( "Dismiss this message" , 'options import', 'presscore' ) . '</a>' 
						. '</div>' 
						. '<div class="dt-button-primary">' 
							. '<a href="' . esc_url( $import_link ) . '" class="button button-primary dt-import-options">' . _x( "Yes, do it!" , 'options import', 'presscore' ) . '</a>' 
