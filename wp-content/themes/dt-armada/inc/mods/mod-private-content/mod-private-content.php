...
?>
    ...
    <p class="field-custom description description-wide pg_menu_restr_wrap">
        <label for="edit-menu-item-custom-<?php echo $item_id; ?>"><?php _e('Which PrivateContent user categories can see the page?', 'presscore') ?></label>
        ...
        <select name="menu-item-pg-hide-<?php echo $item_id; ?>[]" multiple="multiple" class="chzn-select pg_menu_select" data-placeholder="<?php _e('Select categories', 'presscore') ?> .." tabindex="2">
          <option value="all" class="pg_all_field" <?php if(isset($menu_hide[0]) && $menu_hide[0]=='all') echo 'selected="selected"'; ?>>
		  	<?php _e('All', 'presscore') ?>
          </option>
          <option value="unlogged" class="pg_unl_field" <?php if(isset($menu_hide[0]) && $menu_hide[0]=='unlogged') echo 'selected="selected"'; ?>>
		  	<?php _e('Unlogged Users', 'presscore') ?>
          </option>
