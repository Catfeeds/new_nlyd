...
if ( ! function_exists( 'presscore_enqueue_scripts' ) ) :
	...
	function presscore_enqueue_scripts() {
		...
		$dt_local = array(
			'passText' => __( 'To view this protected post, enter the password below:', 'presscore' ),
			'moreButtonText' => array(
				'loading' => __( 'Loading...', 'presscore' ),
			),
...
if ( ! function_exists( 'presscore_comment_id_fields_filter' ) ) :
	...
	function presscore_comment_id_fields_filter( $result ) {
		...
		$comment_buttons = presscore_get_button_html( array( 'href' => 'javascript: void(0);', 'title' => __( 'clear form', 'presscore' ), 'class' => 'clear-form' ) );
		$comment_buttons .= presscore_get_button_html( array( 'href' => 'javascript: void(0);', 'title' => __( 'Submit', 'presscore' ), 'class' => 'dt-btn dt-btn-m' ) );
	...
	function presscore_comment( $comment, $args, $depth ) {
		...
		<li class="pingback">
			<div class="pingback-content">
				<span><?php _e( 'Pingback:', 'presscore' ); ?></span>
				<?php comment_author_link(); ?>
				<?php edit_comment_link( __( '(Edit)', 'presscore' ), ' ' ); ?>
			</div>
		<?php
				break;
			default :
		...
		<li <?php comment_class(); ?> id="comment-<?php comment_ID(); ?>">
			...
			<div class="comment-meta">
				...
				<?php
					...
					printf( __( '%1$s at %2$s', 'presscore' ), get_comment_date(), get_comment_time() ); ?>
				</time>
				<?php edit_comment_link( __( '(Edit)', 'presscore' ), ' ' ); ?>
			</div><!-- .comment-meta -->
			...
			<?php if ( $comment->comment_approved == '0' ) : ?>
				<em><?php _e( 'Your comment is awaiting moderation.', 'presscore' ); ?></em>
				<br />
			<?php endif; ?>
